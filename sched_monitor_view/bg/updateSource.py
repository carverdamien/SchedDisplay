...
def plot(data, event, interval, callback):
	args = (data, event, interval, callback)
...
def target(data, event_selected, interval_selected, callback):
	...
	for i in range(len(Types.INTERVAL)):
		...
		y0 = np.array([])
		y1 = np.array([])
		if i in interval_selected:
			for cpu in data:
				# TODO
				pass
		source_interval_data.append(dict(x0=x0, y0=y0, x1=x1, y1=y1))
