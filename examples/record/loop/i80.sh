...
# PHORONIXES+="blender fio" # rm blender
PHORONIXES+="apache blogbench caffe clpeak cryptsetup darktable dbench ethminer gimp gnupg iozone octave-benchmark openssl redis selenium selenium-top-sites sqlite tesseract-ocr"
PHORONIXES+="aio-stress aircrack-ng aobench aom-av1 apache apache-siege appleseed arrayfire asmfish battery-power-usage blake2 blogbench bork botan brl-cad build-apache build-eigen build-firefox build-gcc build-imagemagick build-linux-kernel build-llvm build-mplayer build-php build-webkitfltk bullet byte cachebench caffe clomp cloverleaf clpeak comd-cl compilebench compress-7zip compress-gzip compress-lzma compress-pbzip2 compress-rar compress-xz compress-zstd core-latency coremark cp2k cpp-perf-bench cpuminer-opt crafty c-ray cryptsetup ctx-clock cyclictest cython-bench dacapobench darktable dav1d dbench dcraw dolfyn ebizzy encode-flac encode-mp3 encode-wavpack espeak ethminer ethr etqw-demo-iqc fahbench ffmpeg ffte fftw fhourstones fs-mark geekbench gimp git glibc-bench gmpbench gnupg go-benchmark gpu-residency gromacs hackbench hdparm-read himeno hint hmmer hpcc hpcg idle idle-power-usage indigobench interbench iozone iperf java-gradle-perf java-jmh java-scimark2 john-the-ripper juliagpu lammps lczero llvm-test-suite luajit lulesh-cl luxmark lzbench mafft mandelbulbgpu mandelgpu mbw mcperf memtier-benchmark mencoder minion mkl-dnn mpcbench m-queens mrbayes multichase mysqlslap namd namd-cuda neatbench nero2d netperf network-loopback nginx node-express-loadtest node-octane noise-level novabench npb n-queens numenta-nab numpy nuttcp octave-benchmark opencv-bench opendwarfs open-porous-media openssl opm-git optcarrot osbench parboil pennant perl-benchmark pgbench php phpbench pjdfstest polybench-c postmark povray powertop-wakeups primesieve psstop pts-self-test pybench pymongo-inserts pyopencl qmcpack qmlbench radiance ramspeed rbenchmark redis renaissance rodinia rust-mandel rust-prime sample-pass-fail sample-program schbench scikit-learn scimark2 selenium selenium-top-sites serial-loopback smallpt smallpt-gpu smart sockperf spec-cpu2017 spec-jbb2015 sqlite startup-time stockfish stream stressapptest stresscpu2 stress-ng sudokut sunflow svt-av1 svt-hevc svt-vp9 swet sysbench systemd-boot-kernel systemd-boot-total systemd-boot-userspace system-decompress-bzip2 system-decompress-gzip system-decompress-tiff system-decompress-xz system-decompress-zlib system-libjpeg system-libxml2 systester tachyon tensorflow tesseract-ocr tinymembench tiobench tjbench tscp t-test1 ttsiod-renderer tungsten unpack-linux vpxenc v-ray x264 x264-opencl x265 xsbench xsbench-cl y-cruncher"
for I in ${!SLP[@]}
do
    SLEEP_STATE=${SLP[$I]}
    SCALING_GOVERNOR=${GOV[$I]}
